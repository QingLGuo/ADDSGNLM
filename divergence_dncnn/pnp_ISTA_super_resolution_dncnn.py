...
def pnp_fbs_superresolution(im_input, im_ref, fx, fy, mask, **opts):
    ...
    psnr_initial = psnr(x, im_ref)
    res = np.zeros(maxiter)
    psnr_values = [psnr_initial]
    """ Main loop. """
    for i in range(maxitr):
        ...
        with torch.no_grad():
            r = model(x)
            r = r.squeeze(dim=0).squeeze(dim=0)
            r = r.cpu().numpy()
        ...
        res_values.append(res[i])

        psnr_value = psnr(x, im_ref)
    ...
    return x, psnr_values, res_values
...
if __name__ == "__main__":
    ...
    out, psnr_values, res_values = pnp_fbs_superresolution(im_noisy, im_orig, fx, fy, mask, **opts)

    imagename1=f'dncnn_{maxiter}_{imagename}'
    output_str = outdir_name + imagename1
    cv2.imwrite(output_str, out * 255.0)

    # ---- results ----
    res = pd.DataFrame(result_iteration_psnr_ssim)
    res.to_csv(f'dncnn_{maxiter}_{imagename}.csv', index=False)
